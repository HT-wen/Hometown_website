...
// ===== Sample product data for Alcohol section (unchanged content) =====
const products = [
...
// ===== Price Label Update (used by inline oninput on the range) =====
...
function filterProducts() {
  ...
  const priceLimit = priceEl ? parseFloat(priceEl.value) : 50;
  const selectedCategory = categoryEl ? categoryEl.value : 'all';

  const filteredProducts = products.filter((product) => {
    const priceMatch = Number(product.price) <= priceLimit;
    const categoryMatch =
      selectedCategory === 'all' || String(product.category).toLowerCase() === selectedCategory;
...
// Attach listeners (with guards)
(function attachFilterListeners(){
  const priceEl = document.getElementById('price-range');
  const categoryEl = document.getElementById('category-filter');
  if (priceEl) priceEl.addEventListener('input', filterProducts);
  if (categoryEl) categoryEl.addEventListener('change', filterProducts);
})();
